...
namespace Sitescope2RemoteWrite.Processing
{
    ...
    public class ZabbixMetricProcessor : BackgroundService
    {
        ...
        public ZabbixMetricProcessor(IConfiguration config, ILogger<ZabbixMetricProcessor> logger, IZabbixMetricQueue zabbixMetricQueue, ITimeSeriesQueue timeSeriesQueue)
        {
            ...
            _getNotKnown = new Timer(GetNotKnown, null, TimeSpan.Zero, TimeSpan.FromSeconds(10));
        }

        private void GetNotKnown(object state)
        {
        ...
        private Dictionary<long, List<Label>> GetMetricLabels (IEnumerable<long> ids)
        {
            ...
            while (reader.Read())
            {
                ...
                for (int i = 1; i<reader.FieldCount; i++)
                {
                    if (!reader.IsDBNull(i))
                    {
                        ...
                        if (name == "__name__")
                        {
                            value = value.Replace("[", "_").Replace("]", "_").Replace(",", "_").Replace("__", "_").Trim('_');
                        }
...
WHERE value_type IN(0, 3) AND FIND_IN_SET(itemid, @ids) != 0";
        private string selectAll_pre_4 = @"SELECT itm.itemid, 
